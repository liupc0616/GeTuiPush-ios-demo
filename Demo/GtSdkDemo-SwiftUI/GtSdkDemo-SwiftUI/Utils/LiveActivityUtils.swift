...
//            }
        ...
        let attri = LiveActivityAttributes(name: "GTSDK")
        do {
            ...
            Task {
                for await state in current.contentStateUpdates {
                    ...
                    print("content state update: tip=\(state.value)")
                }
            }
            Task {
    ...
    static public func updateActivityState(_ value:Int) {
        Task {
            ...
            let state = LiveActivityAttributes.ContentState(value: value)
            let alertConfiguration = AlertConfiguration(title: "Delivery Update ", body: "Delivery Update State to \(state.value)", sound: .default)
